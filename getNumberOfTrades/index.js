...
const getCollectionTrades = async (address, block) => {
    let blockQuery = ""
    if(block) {
    ...
    return getNumberOfTrades(transfers)
}
...
exports.handler = async (event, context) => {
    try {
        ...
        console.log(`Getting number of trades for: ${collection} at block: ${block}`)
        ...
        const trades = await getCollectionTrades(collection, block)

        console.log(`Got number of trades for: ${collection} at block: ${block}`)

        return buildResponse(200, {holdersWithTrades: trades})
    } catch (err) {
        console.log(err);
        throw err;
    }
