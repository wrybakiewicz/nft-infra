...
describe('get average holding time', () => {
    ...
    it('should average holding time after mint', async () => {
        ...
        expect(result[0].totalTokenHoldingTime).to.be.eq(4);
        expect(result[0].averageHoldingTime).to.be.eq(2);

        expect(result[1].holder).to.be.eq(holder2);
        expect(result[1].heldTokensCount).to.be.eq(1);
        expect(result[1].totalTokenHoldingTime).to.be.eq(2);
        expect(result[1].averageHoldingTime).to.be.eq(2);
    });
    ...
    it('should return average holding time after mint and transfers', async () => {
        ...
        expect(result[0].holder).to.be.eq(holder1);
        expect(result[0].heldTokensCount).to.be.eq(2);
        expect(result[0].totalTokenHoldingTime).to.be.eq(6);
        expect(result[0].averageHoldingTime).to.be.eq(3);

        expect(result[1].holder).to.be.eq(holder2);
        expect(result[1].heldTokensCount).to.be.eq(2);
        expect(result[1].totalTokenHoldingTime).to.be.eq(4);
        expect(result[1].averageHoldingTime).to.be.eq(2);
